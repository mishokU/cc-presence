...
Ровно четыре поля и ничего больше:
...
Пингер молчит, пока строка реально не рисуется: нет ни одного рендера
10 минут — он перестаёт пинговать. Поэтому «в консоли» означает, что человек
там сейчас, а не что у него просто включён компьютер.
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/presence/bin/statusline.js"
  }
}
```
...
| | |
|---|---|
| `statusline.js` | рендер строки: только диск, никогда не падает, ~26 мс |
